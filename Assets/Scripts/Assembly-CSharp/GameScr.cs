...
public class GameScr : mScreen, IChatable
{
    ...
    public void readPart()// TODO
    {
        //UnityEngine.Debug.LogError("Read Part");
        ...
        try
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                for (int j = 0; j < parts[i].pi.Length; j++)
                {
                    ...
                    //System.IO.File.AppendAllText(file, qr);
                }

                //string querry = $"INSERT INTO parts(id, type) VALUE ({i}, {type});\n";
    ...
    public void readEfect()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                //string cc = Path.Combine(basePath, $"NR_effect.json");
                //string write = JsonConvert.SerializeObject((object)efs);
    ...
    public void readArrow()
    {
        ...
        try
        {
            ...
            //string basePath = @"C:/Users/Arriety/Desktop/Project/Cha-NRO/resources/data/update_data";
            //string cc = Path.Combine(basePath, $"NR_arrow.json");
            //string write = JsonConvert.SerializeObject((object)arrs);
            //File.AppendAllText(cc, write);
        }
        catch (Exception)
        {
    ...
    public void readDart()
    {
        ...
        try
        {
            ...
            //string basePath = @"C:/Users/Arriety/Desktop/Project/Cha-NRO/resources/data/update_data";
            //string cc = Path.Combine(basePath, $"NR_dart.json");
            //string write = JsonConvert.SerializeObject((object)darts);
            //File.AppendAllText(cc, write);
        }
        catch (Exception ex)
        {
    ...
    public void readSkill()
    {
        ...
        try
        {
            ...
            //UnityEngine.Debug.LogError("Read Skill: " + dataInputStream.available() + " bytes");// 77101
            int num = dataInputStream.readShort();// 157
            int num2 = Skills.skills.size();// 186
            //UnityEngine.Debug.LogError("num size skills: " + num2) ;
            sks = new SkillPaint[num2];
            for (int i = 0; i < num; i++)
            {
                short num3 = dataInputStream.readShort();// id // 57
                if (num3 == 1111)
                {
                    num3 = (short)(num - 1);
                }
                sks[num3] = new SkillPaint();// size 27
                sks[num3].id = num3;
                ...
                string sql = $"INSERT INTO skill_paint (id, effect_happen_on_mob, num_eff) VALUES ({sks[num3].id}, {sks[num3].effectHappenOnMob}, {sks[num3].numEff});\n";
                ...
                for (int j = 0; j < sks[num3].skillStand.Length; j++)
                {
                    ...
                    //string s = $"INSERT INTO skill_stand (id_skill_paint, status, effS0Id, e0dx, e0dy, effS1Id, e1dx, e1dy, effS2Id, e2dx, e2dy, arrowId, adx, ady) VALUES ({sks[num3].id}," +
                    //    $" {sks[num3].skillStand[j].status}," +
                    //    $" {sks[num3].skillStand[j].effS0Id}," +
                    //    $" {sks[num3].skillStand[j].e0dx}," +
                    //    $" {sks[num3].skillStand[j].e0dy}," +
                    //    $" {sks[num3].skillStand[j].effS1Id}," +
                    //    $" {sks[num3].skillStand[j].e1dx}," +
                    //    $" {sks[num3].skillStand[j].e1dy}," +
                    //    $" {sks[num3].skillStand[j].effS2Id}," +
                    //    $" {sks[num3].skillStand[j].e2dx}," +
                    //    $" {sks[num3].skillStand[j].e2dy}," +
                    //    $" {sks[num3].skillStand[j].arrowId}," +
                    //    $" {sks[num3].skillStand[j].adx}," +
                    //    $" {sks[num3].skillStand[j].ady});\n";
                    //File.AppendAllText(file, s);

                }
                sks[num3].skillfly = new SkillInfoPaint[dataInputStream.readByte()];// 14
                for (int k = 0; k < sks[num3].skillfly.Length; k++)
                {
                    sks[num3].skillfly[k] = new SkillInfoPaint();
                    sks[num3].skillfly[k].status = dataInputStream.readByte();// 13
                    sks[num3].skillfly[k].effS0Id = dataInputStream.readShort();// 12
                    ...
                    //string m = $"INSERT INTO skill_fly (id_skill_paint, status, effS0Id, e0dx, e0dy, effS1Id, e1dx, e1dy, effS2Id, e2dx, e2dy, arrowId, adx, ady) VALUES ({sks[num3].id}," +
                    //   $" {sks[num3].skillfly[k].status}," +
                    //   $" {sks[num3].skillfly[k].effS0Id}," +
                    //   $" {sks[num3].skillfly[k].e0dx}," +
                    //   $" {sks[num3].skillfly[k].e0dy}," +
                    //   $" {sks[num3].skillfly[k].effS1Id}," +
                    //   $" {sks[num3].skillfly[k].e1dx}," +
                    //   $" {sks[num3].skillfly[k].e1dy}," +
                    //   $" {sks[num3].skillfly[k].effS2Id}," +
                    //   $" {sks[num3].skillfly[k].e2dx}," +
                    //   $" {sks[num3].skillfly[k].e2dy}," +
                    //   $" {sks[num3].skillfly[k].arrowId}," +
                    //   $" {sks[num3].skillfly[k].adx}," +
                    //   $" {sks[num3].skillfly[k].ady});\n";
                    //File.AppendAllText(f, m);
                }
            }
        }
        catch (Exception ex)
        {
            UnityEngine.Debug.LogError("Loi ham readSkill: " + ex.ToString());
        }
        finally
        {
            try
            ...
            {
                UnityEngine.Debug.LogError("Loi ham readskill: " + ex2.ToString());
            }
        }// 32
    }
    ...
    public void readOk()
    {
        try
        {
            //UnityEngine.Debug.LogError("<readOk><vsData<" + vsData + "==" + vcData);
            //UnityEngine.Debug.LogError("<readOk><vsMap<" + vsMap + "==" + vcMap);
            //UnityEngine.Debug.LogError("<readOk><vsSkill<" + vsSkill + "==" + vcSkill);
            //UnityEngine.Debug.LogError("<readOk><vsItem<" + vsItem + "==" + vcItem);
            if (vsData == vcData && vsMap == vcMap && vsSkill == vcSkill && vsItem == vcItem)
            {
                //UnityEngine.Debug.LogError(vsData + "," + vsMap + "," + vsSkill + "," + vsItem);
                gI().readDart();
                gI().readEfect();
                gI().readArrow();
                gI().readSkill();
                Service.gI().clientOk("public void readOk()");
            }
        }
        catch (Exception ex)
        {
    ...
    public void actionPerform(int idAction, object p)
    {
        ...
        switch (idAction)
        {
            ...
            case 11067:
                ...
                {
                    Service.gI().requestChangeZone(indexSelect);
                    InfoDlg.showWait();
                }
                else
                {
