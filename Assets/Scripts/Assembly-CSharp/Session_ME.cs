...
public class Session_ME : ISession
{
    ...
    private class MessageCollector
    {
        public void run()
        {
            try
            {
                while (connected)
                {
                    ...
                    try
                    {
                        //UnityEngine.Debug.LogError("msg: " + message.command);
                        ...
                        {
                            onRecieveMsg(message);// messageHandler.onMessage(msg);
                        }
                    }
                    catch (Exception)
                    {
            ...
            catch (Exception ex3)
            {
                ...
                //UnityEngine.Debug.LogError(ex3.Message.ToString());
            }
            if (!connected)//  connect = false thi return
            {
                return;
            }
            ...
            {
                //cleanNetwork();
                //UnityEngine.Debug.LogError(">>>>cleanNetwork");
            }
        }
        ...
        private void getKey(Message message)
        {
            try
            {
                ...
                //UnityEngine.Debug.LogError("1 IP: " + GameMidlet.IP2 +  " - Port: " + GameMidlet.PORT2);
                GameMidlet.isConnect2 = ((message.reader().readByte() != 0) ? true : false);
                //if(message.reader.readByte()== 0) return false , else return true;

                //UnityEngine.Debug.LogError("is connect Session me 2: " + GameMidlet.isConnect2);

                if (isMainSession && GameMidlet.isConnect2)
                {
                    GameCanvas.connect2();// connect session me 2
                }
            }
            catch (Exception ex)
            {
                UnityEngine.Debug.LogError("ex: " + ex.StackTrace);
            }
        }
        ...
        private Message readMessage()
        {
            try
            {
                ...
                if (cmd == -32 || cmd == -66 || cmd == 11 || cmd == -67 || cmd == -74 || cmd == -87 || cmd == 66 || cmd == 12)
                {
                    //UnityEngine.Debug.LogError(">>>onRecieveMsg ...!: " + cmd);
                ...
                if (getKeyComplete)
                {
                    ...
                    //UnityEngine.Debug.LogError("Read Key");
                }
                else
                {
    ...
    public void connect(string host, int port)
    {
        ...
        {
            UnityEngine.Debug.Log(">>>return connect ...!" + connected + "  ::  " + connecting);
            return;
        }
        if (mSystem.currentTimeMillis() < timeWaitConnect)
        {
            UnityEngine.Debug.LogError(">>>>chặn việc nó kết nối 2 3 lần liên tục");
            return;
        }
        ...
        //this.host = "14.225.209.121";
        //this.host = "127.0.0.1";
        //this.host = "42.96.4.189";
        this.host = host;
        this.port = port;
        //this.port = 7979;
        ...
        try
        {
            ...
            if (data != null)
            {
                ...
                if (getKeyComplete)
                {
                    ...
                    //Debug.LogError("num 2: " + num2 + " num 3: " + num3);
                }
                else
                {
