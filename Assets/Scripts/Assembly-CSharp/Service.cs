...
public class Service
{
    ...
    public void updateCaption(sbyte gender)
    {
        ...
        try
        {
            message = new Message((sbyte)(-41), " updateCaption gender: " + gender);
    ...
    {
        Message message = new Message((sbyte)(-29), "command: " + command);
    ...
    {
        Message message = new Message((sbyte)(-28),  "type " + command);
        message.writer().writeByte(command);
        return message;
    }
    ...
    public void setClientType()
    {
        ...
        try
        {
            ...
            message.writer().writeByte((mGraphics.zoomewver));
    ...
    {
        UnityEngine.Debug.LogError("changer zone: " + zoneId);
    ...
    {
        Message message = new Message((sbyte)(-28), "char name: " + charname);
        try
        {
            message.writer().writeByte((sbyte)1);
    ...
    public void requestSkill(int skillId)
    {
        ...
        try
        {
            message = messageNotMap(9);// deo dung ne
    ...
    public void selectSkill(int skillTemplateId)
    {
        ...
        try
        {
            message = new Message((sbyte)34, " SELECT SKILL " + skillTemplateId);
            message.writer().writeShort(skillTemplateId);
            session.sendMessage(message);
        }
        catch (Exception ex)
    ...
    public void chat(string text)
    {
        ...
        try
        {
            ...
            string trimText = text.Trim().ToLower();
            string[] e = trimText.Split(' ');
            ...
            switch (e[0])
            {
                case "beo":
                    ...
                    break;
                case "k":
                    string[] mew = trimText.Split(' ');
                    if (mew.Length == 2)
                    {
                        if (int.TryParse(mew[1], out int idzone))
                        {
                            this.requestChangeZone(idzone);
                            GameScr.info1.addInfo("Bạn đã đổi sang khu " + idzone, 0);
                        }
                    }
                    break;
                case "i":
                    string[] imagebyname = trimText.Split(' ');
                    if (imagebyname.Length == 4)
                    {
                        if (int.TryParse(imagebyname[1], out int type) && int.TryParse(imagebyname[2], out int from) && int.TryParse(imagebyname[3], out int to))
                        {
                            this.voidKeoImageByNameAsync((sbyte)type, from, to);
                        }
                    }
                    break;
                case "eff":
                    string[] eff = trimText.Split(' ');
                    if (eff.Length == 3)
                    {
                        if (int.TryParse(eff[1], out int from) && int.TryParse(eff[2], out int to))
                        {
                            this.keoEff(from, to);
                        }
                    }
                    break;
                case "bg":
                    string[] bg = trimText.Split(' ');
                    if (bg.Length == 3)
                    {
                        if (int.TryParse(bg[1], out int from) && int.TryParse(bg[2], out int to))
                        {
                            this.keoBg(from, to);
                        }
                    }
                ...
                case "part":
                    ...
                    break;
                case "mob":
                    string[] mob = trimText.Split(' ');
                    if (mob.Length == 3)
                    {
                        if (int.TryParse(mob[1], out int from) && int.TryParse(mob[2], out int to))
                        {
                            this.keoMob(from, to);
                        }
                    }
    ...
    private void keoMap(int form, int to)
    {
        new Thread(delegate ()
        {
            for (int i = form; i <= to; i++)
            {
                this.requestMaptemplate(i);
            }
        }).Start();
    }

    private void keoMob(int form, int to)
    {
        new Thread(delegate ()
        {
            for (int i = form; i <= to; i++)
    ...
    private void voidKeoImageByNameAsync(sbyte type, int from, int to)
    {
        new Thread(delegate ()
        {
            switch (type)
            {
                case 0:
                    ...
                    {
                        string mount = "mount_" + j + "_0";
                        string mount1 = "mount_" + j + "_1";
                        this.getImgByName(mount);
                        this.getImgByName(mount1);
                        UnityEngine.Debug.LogError("đang kéo image by name type 1");
                        Thread.Sleep(300);
                    }
                    break;
                case 1:
                    for (int j = from; j <= to; j++)
                    {
                        string aura = "aura_" + j + "_0";
                        string aura1 = "aura_" + j + "_1";
                        this.getImgByName(aura);
                        this.getImgByName(aura1);
                        Thread.Sleep(300);
                    }
                    break;
                case 2:
                    for (int j = from; j <= to; j++)
                    {
                        string setEff = "set_eff_" + j + "_0";
                        string setEff1 = "set_eff_" + j + "_1";
                        this.getImgByName(setEff);
                        this.getImgByName(setEff1);
                        Thread.Sleep(300);
                    }
                    break;
                case 3:
                    for (int j = from; j <= to; j++)
                    {
                        string banner = "banner_" + j;
                        this.getImgByName(banner);
                        Thread.Sleep(300);
                    }
                    break;
                case 4:
                    for (int j = from; j <= to; j++)
                    {
                        string hat_sau = "hat_sau_" + j;
                        string hat_truoc = "hat_truoc_" + j;
                        string hat_truoc_ngang = "hat_truoc_ngang_" + j;
                        string hat_sau_ngang = "hat_sau_ngang_" + j;
                        this.getImgByName(hat_sau);
                        this.getImgByName(hat_truoc);
                        this.getImgByName(hat_truoc_ngang);
                        this.getImgByName(hat_sau_ngang);
                        Thread.Sleep(300);
                    }
                    break;
                case 5:
                ...
                default:
                    GameScr.info1.addInfo("Không có type Image By Name này", 0);
                    return;
            }
            GameScr.info1.addInfo("Kéo image by name type: " + type, 0);
        }).Start();
    }

    private void keoEff(int form, int to)
    {
        new Thread(delegate ()
        {
            for (int i = form; i <= to; i++)
            {
                this.getEffData((short)i);
                Thread.Sleep(500);
            }
        }).Start();
    }

    private void keoIcon(int form, int to)
    {
        new Thread(delegate ()
        {
            for (int i = form; i <= to; i++)
            {
                //SmallImage.imgNew[i] = new Small(SmallImage.imgEmpty, i);
                this.requestIcon(i);
                Thread.Sleep(300);
                //SmallImage.imgNew[i] = null;
            }
        }).Start();
    }

    private void keoBg(int form, int to)
    {
        new Thread(delegate ()
        {
            for (int i = form; i <= to; i++)
            {
                this.getBgTemplate((short)i);
                Thread.Sleep(300);
            }
        }).Start();
    }
    ...
    public void updateItem(String method)
    {
        Message message = null;
        try
        {
        ...
        {
            message.cleanup();
            //UnityEngine.Debug.LogError("updateItem " + method);
        }

    }

    public void clientOk(string method)
    {
        //UnityEngine.Debug.LogError("clientOk " + method);
        Message message = null;
        try
        {
    ...
    public void clearTask()
    {
        ...
        try
        {
            message = messageNotMap(17);// deo dung ne
            session.sendMessage(message);
        }
        catch (Exception ex)
        {
    ...
    public void requestIcon(int id)
    {
        ...
        var hehe = $"Called by: {method.DeclaringType.FullName}.{method.Name}";
        //UnityEngine.Debug.LogError("Request Icon: " + id + "-" + hehe);
        ...
        try
        {
            ...
            //UnityEngine.Debug.LogError("Dang Keo Id: " + id);
            session.sendMessage(message);
            //UnityEngine.Debug.LogError(">>>>>>>>>>>>>REQUEST ICON " + id + "  isConnected:" + Controller.isGet_CLIENT_INFO);
            session = Session_ME.gI();
        }
        catch (Exception ex)
        {
    ...
    public void doConvertUpgrade(int index1, int index2, int index3)
    {
        ...
        try
        {
            message = messageNotMap(33);// Cai nay deo dung ne
    ...
    public void inviteClanDun(string name)
    {
        ...
        try
        {
            message = messageNotMap(34);// Cai nay deo dung ne
            message.writer().writeUTF(name);
            session.sendMessage(message);
        }
        catch (Exception ex)
    ...
    public void inputNumSplit(int indexItem, int numSplit)
    {
        ...
        try
        {
            message = messageNotMap(40);// Cai nay deo dung ne
    ...
    public void activeAccProtect(int pass)
    {
        ...
        try
        {
            message = messageNotMap(37);// maajt khau bao ve
    ...
    public void updateActive(int passOld, int passNew)
    {
        ...
        try
        {
            message = messageNotMap(38);// cai nay deo dung ne
    ...
    public void openLockAccProtect(int pass2)
    {
        ...
        try
        {
            message = messageNotMap(39);//  mo khoa mat khau cap 2
    ...
    public void finishUpdate(string method)
    {
        Message message = null;
        try
        {
            message = new Message((sbyte)(-38), method);
            session.sendMessage(message);
        }
        catch (Exception ex)
        {
    ...
    {
        // vResourceIndex = null
        //UnityEngine.Debug.LogError("request resource action= " + action);
        Message message = null;
        try
        {
            message = new Message((sbyte)(-74));
            message.writer().writeByte(action);// write byte action
    ...
    public void imageSource(MyVector vID)
    {
        ...
        try
        {
            ...
            message = new Message((sbyte)(-111), "vID: " + vID.size());
