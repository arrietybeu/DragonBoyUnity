...
public class Controller : IMessageHandler
{
    ...
    public static bool isSendItem = true;
    ...
    public void onMessage(Message msg)
    {
        ...
        try
        {
            ...
            using (StreamWriter writer = new StreamWriter(filePath, true))
            {
                ...
                writer.WriteLine($"Server gửi: [{msg.command}] => {string.Join(", ", cmdNames)}");
            }
            ...
            switch (msg.command)
            {
                ...
                case -82:
                    {
                        ...
                        for (int num64 = 0; num64 < b28; num64++)
                        {
                            ...
                            //string queryTileSet = $"INSERT INTO tile_set_info (id, so_tile_type) VALUES ({num64}, {b29});\n";
                            //File.AppendAllText(filepath, queryTileSet);

                            for (int num65 = 0; num65 < b29; num65++)
                            {
                                TileMap.tileType[num64][num65] = msg.reader().readInt();
                                int tileTypeVal = TileMap.tileType[num64][num65];
                                ...
                                for (int num66 = 0; num66 < b30; num66++)
                                {
                                    ...
                                    int indexVal = TileMap.tileIndex[num64][num65][num66];
                                }

                                //string queryTileType =
                                //  "INSERT INTO map_tile_type (id, tile_set_id, tile_type_value, so_index, index_value) " +
                                //  $"VALUES ({num65}, {num64}, {tileTypeVal}, {b30}, '{JsonConvert.SerializeObject((object)TileMap.tileIndex[num64][num65])}');\n";
                ...
                case -77:
                    {
                        ...
                        //Debug.LogError("smaill version: " + num115);
                ...
                case -74:
                    {
                        //Debug.LogError("1>>GET_IMAGE_SOURCE = " + msg.reader().available());
                        ...
                        if (!GameCanvas.isGetResourceFromServer())
                        {
                            ...
                            if (Rms.loadRMSString("acc") != null || Rms.loadRMSString("userAo"
                                + ServerListScreen.ipSelect) != null)
                            {
                        ...
                        //Debug.LogError("2>GET_IMAGE_SOURCE = " + b40);
                        string freefire = "E:\\log.txt";
                        using (StreamWriter writer = new StreamWriter(freefire, true))
                        {
                            writer.WriteLine($"Server gửi: [MSG GET_IMAGE_SOURCE] => TYPES: {b40}");
                        }
                        switch (b40)
                        {
                            case 0:
                                int num92 = msg.reader().readInt();// x4 13860119 ||  x2 5870702 || x1 467066 || x3 10845624
                                //Debug.LogError("Res Size: " + num92);
                                string text3 = Rms.loadRMSString("ResVersion");
                                int num93 = ((text3 == null || !(text3 != string.Empty)) ? (-1) : int.Parse(text3));
                                //Debug.LogError(">>>GET_IMAGE_SOURCE: version>> " + text3 + " <> " + num93 + "!=" + num92);
                                ...
                                else
                                {
                                    ...
                                    //Debug.LogError(">>>vo ne: " + GameCanvas.currentScreen);
                            ...
                            case 2:
                                try
                                {
                                    ...
                                    //    string cc = Path.Combine(basePath, $"x{mGraphics.zoomewver}", $"beo/{original}.png");
                                    //    File.WriteAllBytes(cc, Array.ConvertAll(data4, (sbyte a) => (byte)a));
                                    //}
                                    //else
                                    //{
                                    //    string basePath = @"C:/Users/Arriety/Desktop/Project/Cha-NRO/resources";
                                    //    string cc = Path.Combine(basePath, $"x{mGraphics.zoomewver}", "beo", $"{array8[array8.Length - 1]}.png");
                                ...
                                {
                                    Debug.LogError("Loi get res: " + ex.StackTrace);
                ...
                case -67:
                    {
                        ...
                        //UnityEngine.Debug.LogError("RECIEVE  hinh small: " + num156);
                        sbyte[] array17 = null;
                        try
                        {
                            array17 = NinjaUtil.readByteArrayIcon(num156, msg);
                ...
                //case -66:
                //    {
                //        short id3 = msg.reader().readShort();// id eff
                //        sbyte[] data5 = NinjaUtil.readByteDataEff(msg, id3);
                //        EffectData effDataById = Effect.getEffDataById(id3);
                //        //Debug.LogError("Server gui Eff id: " + id3);
                //        sbyte b66 = msg.reader().readSByte();// type l gi do
                //        if (b66 == 0)
                //        {
                //            if (data5 != null)
                //            {
                ...
                //            }
                //        }
                //        // chan keo eff roi den day ko call duoc nua
                //        sbyte[] array15 = NinjaUtil.readByteImageEff(msg, id3);
                //        effDataById.img = Image.createImage(array15, 0, array15.Length);
                //        break;
                //    }
                case -32:
                ...
                case 43:
                    ...
                    {
                        //Char.myCharz().taskMaint.count = 1906;
                    }
                    break;
                case 90:
                ...
                case 32:
                    {
                        ...
                        int num76 = msg.reader().readShort();// id npc
                        for (int num77 = 0; num77 < GameScr.vNpc.size(); num77++)
                        {
                            ...
                            {
                                string chat = msg.reader().readUTF();// npc say
                                string[] array6 = new string[msg.reader().readByte()];// type gi do
                                for (int num78 = 0; num78 < array6.Length; num78++)
                                {
                                    array6[num78] = msg.reader().readUTF();// list tab npc
                                }
                ...
                case -24:
                    ...
                    //Debug.LogError("zone id: " + TileMap.zoneID);
                ...
                case -31:
                    {
                        ...
                        for (int num62 = 0; num62 < num61; num62++)
                        {
                            ...
                            string querry = $"INSERT INTO item_background(id, image, layer, dx, dy, tile_x, tile_y) " +
                                $"VALUE ({bgItem.id}, {bgItem.idImage}, {bgItem.layer}, {bgItem.dx}, {bgItem.dy}, '{JsonConvert.SerializeObject((object)bgItem.tileX)}', '{JsonConvert.SerializeObject((object)bgItem.tileY)}');\n";
                            //File.AppendAllText(riete, querry);
                        ...
                        if (num2 != Char.myCharz().charID)
                        {
                            ...
                            for (num = 0; num < array.Length; num++)
                            {
                                ...
                                if (num3 == Char.myCharz().charID)
                                {
                                    ...
                                    {
                                        Service.gI().requestChangeZone(-1);
        ...
        {
            Debug.LogError("BUG: " + msg.command + " msg loi: " + ex41.StackTrace);
            //Debug.LogError($"[Controller] [error] " + ex41.StackTrace + " msg: " + msg + " cause " + ex41.Data);
        }
        finally
        {
            if (msg != null)
    ...
    private void createSkill(myReader dis)
    {
        try
        {
            ...
            GameScr.gI().sOptionTemplates = new SkillOptionTemplate[dis.readByte()];
            for (int i = 0; i < GameScr.gI().sOptionTemplates.Length; i++)
            {
                ...
                GameScr.gI().sOptionTemplates[i].name = dis.readUTF();
            }

            GameScr.nClasss = new NClass[dis.readByte()];
            for (int j = 0; j < GameScr.nClasss.Length; j++)
            {
                GameScr.nClasss[j] = new NClass();
                GameScr.nClasss[j].classId = j;
                GameScr.nClasss[j].name = dis.readUTF();
                GameScr.nClasss[j].skillTemplates = new SkillTemplate[dis.readByte()];

                //var content = $"skill class: {GameScr.nClasss[j].classId}" +
                //    $" : template size: {GameScr.nClasss[j].skillTemplates.Length}\n";
                //File.AppendAllText(log, content);
                int count = 0;
                for (int k = 0; k < GameScr.nClasss[j].skillTemplates.Length; k++)
                {
                    GameScr.nClasss[j].skillTemplates[k] = new SkillTemplate();
                    GameScr.nClasss[j].skillTemplates[k].id = dis.readByte();
                    GameScr.nClasss[j].skillTemplates[k].name = dis.readUTF();
                    GameScr.nClasss[j].skillTemplates[k].maxPoint = dis.readByte();
                    GameScr.nClasss[j].skillTemplates[k].manaUseType = dis.readByte();
                    GameScr.nClasss[j].skillTemplates[k].type = dis.readByte();
                    GameScr.nClasss[j].skillTemplates[k].iconId = dis.readShort();
                    GameScr.nClasss[j].skillTemplates[k].damInfo = dis.readUTF();
                    int lineWidth = 130;
                    ...
                    var desc = dis.readUTF();
                    GameScr.nClasss[j].skillTemplates[k].description = mFont.tahoma_7_green2.splitFontArray(desc, lineWidth);
                    GameScr.nClasss[j].skillTemplates[k].skills = new Skill[dis.readByte()];

                    //var skilltemplate = $"skill id: {GameScr.nClasss[j].skillTemplates[k].id} size: {GameScr.nClasss[j].skillTemplates[k].skills.Length}\n";
                    //File.AppendAllText(log, skilltemplate);

                    for (int l = 0; l < GameScr.nClasss[j].skillTemplates[k].skills.Length; l++)
                    {
                        GameScr.nClasss[j].skillTemplates[k].skills[l] = new Skill();
                        GameScr.nClasss[j].skillTemplates[k].skills[l].skillId = dis.readShort();
                        GameScr.nClasss[j].skillTemplates[k].skills[l].template = GameScr.nClasss[j].skillTemplates[k];
                        GameScr.nClasss[j].skillTemplates[k].skills[l].point = dis.readByte();
                        GameScr.nClasss[j].skillTemplates[k].skills[l].powRequire = dis.readLong();
                        GameScr.nClasss[j].skillTemplates[k].skills[l].manaUse = dis.readShort();
                        GameScr.nClasss[j].skillTemplates[k].skills[l].coolDown = dis.readInt();
                        GameScr.nClasss[j].skillTemplates[k].skills[l].dx = dis.readShort();
                        GameScr.nClasss[j].skillTemplates[k].skills[l].dy = dis.readShort();
                        GameScr.nClasss[j].skillTemplates[k].skills[l].maxFight = dis.readByte();
                        GameScr.nClasss[j].skillTemplates[k].skills[l].damage = dis.readShort();
                        GameScr.nClasss[j].skillTemplates[k].skills[l].price = dis.readShort();
        ...
        {
            Debug.LogError("BUG: " + ex.StackTrace);
        }
    }

    private void createMap(myReader dis)
    {
        try
        {
            ...
            for (int i = 0; i < TileMap.mapNames.Length; i++)
            {
                ...
                string sql = $"INSERT INTO map_template(id, NAME) VALUES ({i}, '{TileMap.mapNames[i]}');\n";
            ...
            for (sbyte b = 0; b < Npc.arrNpcTemplate.Length; b++)
            {
                ...
                Npc.arrNpcTemplate[b].name = dis.readUTF();
                Npc.arrNpcTemplate[b].headId = dis.readShort();
                Npc.arrNpcTemplate[b].bodyId = dis.readShort();
                Npc.arrNpcTemplate[b].legId = dis.readShort();
                Npc.arrNpcTemplate[b].menu = new string[dis.readByte()][];
                for (int j = 0; j < Npc.arrNpcTemplate[b].menu.Length; j++)
                {
                    Npc.arrNpcTemplate[b].menu[j] = new string[dis.readByte()];
                    for (int k = 0; k < Npc.arrNpcTemplate[b].menu[j].Length; k++)
                    {
                        Npc.arrNpcTemplate[b].menu[j][k] = dis.readUTF();
                        //Debug.LogError("npc say: " + Npc.arrNpcTemplate[b].menu[j][k]);
                    }
                }

                string sql = $"INSERT INTO npc_template(id, NAME, head, body, leg, menu) VALUES " +
                  $"({Npc.arrNpcTemplate[b].npcTemplateId}," +
                  $"'{Npc.arrNpcTemplate[b].name}'," +
                  $"{Npc.arrNpcTemplate[b].headId}," +
                  $"{Npc.arrNpcTemplate[b].bodyId}," +
                  $"{Npc.arrNpcTemplate[b].legId}," +
                  $"'{JsonConvert.SerializeObject((object)Npc.arrNpcTemplate[b].menu)}'" +
                  $");\n";
                //File.AppendAllText(f, sql);
            }
            Mob.arrMobTemplate = new MobTemplate[dis.readShort()];// 7

            for (int l = 0; l < Mob.arrMobTemplate.Length; l++)
            {
                Mob.arrMobTemplate[l] = new MobTemplate();
                Mob.arrMobTemplate[l].mobTemplateId = l;
                Mob.arrMobTemplate[l].type = dis.readByte();
                Mob.arrMobTemplate[l].name = dis.readUTF();
                Mob.arrMobTemplate[l].hp = dis.readLong();
                Mob.arrMobTemplate[l].rangeMove = dis.readByte();
                Mob.arrMobTemplate[l].speed = dis.readByte();
                Mob.arrMobTemplate[l].dartType = dis.readByte();
                string sql = $"INSERT INTO monster_template(id, type, NAME, hp, range_move, speed, dart_type) VALUES ({Mob.arrMobTemplate[l].mobTemplateId}," +
                    $"{Mob.arrMobTemplate[l].type}," +
                    $"'{Mob.arrMobTemplate[l].name}'," +
                    $"{Mob.arrMobTemplate[l].hp}," +
                    $"{Mob.arrMobTemplate[l].rangeMove}," +
                    $"{Mob.arrMobTemplate[l].speed}," +
                    $"{Mob.arrMobTemplate[l].dartType});\n";
    ...
    private void createData(myReader dis, bool isSaveRMS)
    {
        GameScr.vcData = dis.readByte();
        if (isSaveRMS)
        {
            Rms.saveRMS("NR_dart", NinjaUtil.readByteArrayNR_dart(dis));
            Rms.saveRMS("NR_arrow", NinjaUtil.readByteArrayNR_arrow(dis));
            Rms.saveRMS("NR_effect", NinjaUtil.readByteArrayNR_effect(dis));
            Rms.saveRMS("NR_image", NinjaUtil.readByteArray(dis));
            Rms.saveRMS("NR_part", NinjaUtil.readByteArray(dis));
            Rms.saveRMS("NR_skill", NinjaUtil.readByteArray(dis));
    ...
    public void loadInfoMap(Message msg)
    {
        try
        {
            ...
            //Debug.LogError("sl item on map " + empty + "\n");
        ...
        {
            Debug.LogError(">>>> Loadmap khong thanh cong [" + ex.StackTrace + "]");
    ...
    public void keyValueAction(string key, string value)
    {
        if (key.Equals("eff"))
        {
            ...
            // cắt chuỗi string lấy value sau mỗi .
    ...
    public void messageNotMap(Message msg)
    {
        ...
        try
        {
            ...
            using (StreamWriter writer = new StreamWriter(filePath, true))
            {
                writer.WriteLine($"Server gửi: [MSG NOT MAP] => TYPES: {b}");
            }
                ...
                case 4:
                    {
                        ...
                        //Debug.LogError("vsItem= " + GameScr.vsItem + " vcItem= " + GameScr.vcItem);
                        if (GameScr.vsItem != GameScr.vcItem)
                        {
                            GameScr.isLoadAllData = false;
                            Service.gI().updateItem("GameScr.vsItem != GameScr.vcItem");
                        }
                        else
                        {
                            try
                            {
                                //Debug.LogError("load item local");
                            ...
                            {
                                GameScr.vcItem = -1;
                                Service.gI().updateItem("Exception");
                            }
                            try
                            {
                        ...
                        {
                            Service.gI().clientOk("if (!GameScr.isLoadAllData)");
                        }
                        sbyte b4 = msg.reader().readByte();
                ...
                case 6:
                    {
                        ...
                        msg.reader().mark(500000);// đánh dấu vị trí dữ liệu 
                        createMap(msg.reader());// chua phat trien
                        msg.reader().reset();
    ...
    public void messageNotLogin(Message msg)
    {
        try
        {
            ...
            using (StreamWriter writer = new StreamWriter(filePath, true))
            {
                writer.WriteLine($"Server gửi: [MSG NOT LOGIN] => TYPES: {b}");
            }
    ...
    public void messageSubCommand(Message msg)
    {
        try
        {
            ...
            {
                var cmdNames = CmdReflection.GetCommandNames(msg.command);
                writer.WriteLine($"---messageSubCommand : {b}");
            }
            switch (b)
            {
                case 63:
    ...
    private void readGetImgByName(Message msg)
    {
        try
        {
            ...
            //Debug.LogError("server gui imagebyname: " + name);
            sbyte[] array = null;
            array = NinjaUtil.readByteImageByName(msg, name);
    ...
    private void createItemNew(myReader d)
    {
        try
        {
            ...
            //Debug.LogError("createItemNew server not map ");
    ...
    private void loadItemNew(myReader d, sbyte type, bool isSave)
    {
        ...
        try
        {
            ...
            type = d.readByte();
            //Debug.LogWarning("GET UPDATE_ITEM " + d.available() + " bytes for type: " + type + " isSave:" + isSave);

            //Debug.LogError(GameScr.vcItem + ":<<GameScr.vcItem >>>>>>loadItemNew: " + type + "  isSave:" + isSave);
            if (type == 0)
            {
            ...
            else if (type == 1)
            {
                ...
                int num = d.readShort();
                //Debug.LogError("num size item: " + num);
                for (int j = 0; j < num; j++)
                {
                    ItemTemplate it = new ItemTemplate((short)j,
                        d.readByte(),
                        d.readByte(),
                        d.readUTF(),
                        d.readUTF(),
        ...
        {
            Debug.LogError("loi type: " + type + " " + ex2.StackTrace);
        }
    }
    ...
    //    {
    //        dis.mark(1000000);
    //        GameScr.vcItem = dis.readByte();
    //        type = dis.readByte();
    //        //Debug.LogError(GameScr.vcItem + ":<<GameScr.vcItem >>>>>>loadItemNew: " + type + "  isSave:" + isSave);
    //        switch (type)
    //        {
    //            case 0:
    //                GameScr.gI().iOptionTemplates = new ItemOptionTemplate[dis.readShort()];
    //                for (int i = 0; i < GameScr.gI().iOptionTemplates.Length; i++)
    //                {
    //                    GameScr.gI().iOptionTemplates[i] = new ItemOptionTemplate();
    //                    GameScr.gI().iOptionTemplates[i].id = i;
    //                    GameScr.gI().iOptionTemplates[i].name = dis.readUTF();
    //                    GameScr.gI().iOptionTemplates[i].type = dis.readByte();
    //                    string query = $"INSERT INTO item_option_template (id, name, type) VALUES ({GameScr.gI().iOptionTemplates[i].id}," +
    //                        $" '{GameScr.gI().iOptionTemplates[i].name}'," +
    //                        $" {GameScr.gI().iOptionTemplates[i].type});\n";
    //                    //File.AppendAllText(sql, query);
    //                }
    //                if (isSave)
    //                {
    //                    dis.reset();
    //                    sbyte[] data = new sbyte[dis.available()];
    //                    dis.readFully(ref data);
    //                    Rms.saveRMS("NRitem0", data);
    //                }
    //                break;
    //            case 1:
    ...
    //                {
    //                    dis.reset();
    //                    sbyte[] data2 = new sbyte[dis.available()];
    //                    dis.readFully(ref data2);
    //                    Rms.saveRMS("NRitem1", data2);
    //                    sbyte[] data3 = new sbyte[1] { GameScr.vcItem };
    //                    Rms.saveRMS("NRitemVersion", data3);
    //                }
    ...
    //                break;
    //            case 2:
    //                Debug.LogError("hihihihihih server send type 2 cho msg load Item (not map) ");
    //                break;
    //            case 100:
    //                Char.Arr_Head_2Fr = readArrHead(dis);
    //                if (isSave)
    //                {
    //                    dis.reset();
    //                    sbyte[] data4 = new sbyte[dis.available()];
    //                    dis.readFully(ref data4);
    //                    Rms.saveRMS("NRitem100", data4);
    //                }
    //                break;
    //            case 101:
    ...
    //                    for (int k = 0; k < num2; k++)
    //                    {
    //                        short num3 = dis.readShort();
    //                        Char.Arr_Head_FlyMove[k] = num3;
    //                        //string qezi = $"INSERT INTO item_head_move (lồn, cặc) VALUES ({num2}, {Char.Arr_Head_FlyMove[k]});\n";
    //                        //File.AppendAllText(keke, qezi);
    //                    }
    //                    if (isSave)
    //                    {
    //                        dis.reset();
    //                        sbyte[] data5 = new sbyte[dis.available()];
    //                        dis.readFully(ref data5);
    //                        Rms.saveRMS("NRitem101", data5);
    //                    }
    //                }
    //                catch (Exception)
    ...
    private int[][] readArrHead(myReader d)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                string query = $"INSERT INTO item_head (id, head1, head2) VALUES ({i}, {array[i][0]}, {array[i][1]});\n";
                //File.AppendAllText(sql, query);
            }
        }
        catch (Exception)
